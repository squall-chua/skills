...
For up-to-date signatures, use `godig symbol doc github.com/samber/lo <Symbol>` (→ `references/pkg-go-dev/GUIDE.md`) or check [pkg.go.dev/github.com/samber/lo](https://pkg.go.dev/github.com/samber/lo). Context7 is a fallback if a symbol is not indexed there.
