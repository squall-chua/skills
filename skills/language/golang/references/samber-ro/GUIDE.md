...
- → See `references/samber-lo/GUIDE.md` for finite slice transforms (Map, Filter, Reduce, GroupBy) — use lo when data is already in a slice
- → See `references/samber-mo/GUIDE.md` for monadic types (Option, Result, Either) that compose with ro pipelines
- → See `references/samber-hot/GUIDE.md` for in-memory caching (also available as an ro plugin)
- → See `references/concurrency/GUIDE.md` for goroutine/channel patterns when reactive streams are overkill
- → See `references/observability/GUIDE.md` for monitoring reactive pipelines in production
