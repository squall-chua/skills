> **Community default.** A company skill that explicitly supersedes `references/refactoring/GUIDE.md` takes precedence.
...
- **Load `references/security/GUIDE.md` (and `golang-safety` for internal-correctness risk) whenever a step changes code logic, not just its shape.**
...
- → See `references/naming/GUIDE.md` for what to rename identifiers _to_ — this skill owns _how_ to apply a rename safely at scale.
- → See `references/project-layout/GUIDE.md` for target directory/package layout — this skill owns the mechanics of moving code there without breaking callers.
- → See `references/modernize/GUIDE.md` for version-driven idiom updates (`interface{}`→`any`, `slices`/`maps`) — a distinct concern from structural refactoring, though it shares the same tool-first discipline.
- → See `references/code-style/GUIDE.md` for control-flow clarity and function-shape rules this skill helps you apply mechanically.
- → See `references/design-patterns/GUIDE.md` for target patterns (options struct, DI, consumer-side interfaces) this skill helps you migrate toward.
- → See `references/testing/GUIDE.md` for the test-writing practices that make the safety net in this skill trustworthy.
- → See `references/lint/GUIDE.md` for configuring `golangci-lint`, run here only as a post-step verification gate.
- → See `references/security/GUIDE.md` (and `golang-safety`) for reviewing any step that changes code logic, not just its shape.
