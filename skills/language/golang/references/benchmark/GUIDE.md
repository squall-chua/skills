...
- → See `references/performance/GUIDE.md` for optimization patterns to apply after measuring ("if X bottleneck, apply Y")
- → See `references/troubleshooting/GUIDE.md` for pprof setup on running services (enable, secure, capture), Delve debugger, GODEBUG flags, root cause methodology
- → See `references/observability/GUIDE.md` for everyday always-on monitoring, continuous profiling (Pyroscope), distributed tracing (OpenTelemetry)
- → See `references/testing/GUIDE.md` for general testing practices
