...
Requires `import _ "net/http/pprof"` (see `references/troubleshooting/GUIDE.md` for secure setup):
...
Short-lived allocations creating GC churn. Objects are allocated and freed rapidly — each one is cheap individually but the aggregate volume triggers frequent GC cycles. Common sources: `fmt.Errorf` in hot paths (allocates every call), interface boxing (`any` arguments), string-to-byte conversions, slice growth without preallocation. → See `references/performance/GUIDE.md` for allocation reduction patterns.
...
Allocation rate is the bottleneck, not computation. The Go runtime is spending more time allocating and collecting garbage than running your code. Switch to the `alloc_objects` heap profile to find which functions allocate the most, then → See `references/performance/GUIDE.md` for reduction patterns.
