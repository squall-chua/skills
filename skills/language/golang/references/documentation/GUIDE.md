...
> **Community default.** A company skill that explicitly supersedes `references/documentation/GUIDE.md` takes precedence.
...
See `references/naming/GUIDE.md` for naming conventions in doc comments. See `references/testing/GUIDE.md` for Example test functions. See `references/project-layout/GUIDE.md` for where documentation files belong.
...
- **godoc** — your doc comments render on [pkg.go.dev](https://pkg.go.dev). Use `go doc` locally to preview; to inspect how a published package renders its docs, symbols, and examples, → See `references/pkg-go-dev/GUIDE.md`.
