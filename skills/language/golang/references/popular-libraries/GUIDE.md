...
This skill is not exhaustive. Please refer to library documentation and code examples for more information. When exploring a candidate library, → See `references/pkg-go-dev/GUIDE.md` (`godig`) for docs, symbols, versions, importers, and known vulnerabilities — prefer it over Context7 for Go package facts. Once a candidate is added to your build, → See `references/gopls/GUIDE.md` (`gopls`) to browse its actual resolved source and compare candidates side by side. Context7 remains a fallback for docs not indexed on pkg.go.dev.
...
- → See `references/dependency-management/GUIDE.md` for adding, auditing, and managing dependencies
- → See `references/pkg-go-dev/GUIDE.md` to vet a candidate library on pkg.go.dev — versions, importers, licenses, and known vulnerabilities — before adopting it
- → See `references/samber-do/GUIDE.md` for samber/do dependency injection details
- → See `references/samber-oops/GUIDE.md` for samber/oops error handling details
- → See `references/stretchr-testify/GUIDE.md` for testify testing details
- → See `references/grpc/GUIDE.md` for gRPC implementation details
