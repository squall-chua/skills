...
> **Community default.** A company skill that explicitly supersedes `references/code-style/GUIDE.md` takes precedence.

# Go Code Style

Style rules that require human judgment — linters handle formatting, this skill handles clarity. For naming see `references/naming/GUIDE.md`; for design patterns see `references/design-patterns/GUIDE.md`; for struct/interface design see `references/structs-interfaces/GUIDE.md`.
...
- Functions SHOULD have **≤4 parameters**. Beyond that, use an options struct (see `references/design-patterns/GUIDE.md`).
...
- **Unexport aggressively** — you can always export later; unexporting is a breaking change. → See `references/gopls/GUIDE.md` to unexport safely — its rename updates every call site atomically and refuses the change when lowercasing a method would break interface satisfaction, a breakage grep/sed silently ships.
...
Many rules are enforced automatically: `gofmt`, `gofumpt`, `goimports`, `gocritic`, `revive`, `wsl_v5`. → See the `references/lint/GUIDE.md`.

## Cross-References

- → See the `references/naming/GUIDE.md` for identifier naming conventions
- → See the `references/structs-interfaces/GUIDE.md` for pointer vs value receivers, interface design
- → See the `references/design-patterns/GUIDE.md` for functional options, builders, constructors
- → See the `references/lint/GUIDE.md` for automated formatting enforcement
- → See `references/continuous-integration/GUIDE.md` for automated AI-driven code review in CI using these guidelines
- → See `references/refactoring/GUIDE.md` for mechanically applying guard-clause conversion, function extraction, and options-struct migration safely across many call sites once a review surfaces violations at scale
