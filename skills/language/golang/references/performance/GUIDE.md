...
1. **Profile before optimizing** — intuition about bottlenecks is wrong ~80% of the time. Use pprof to find actual hot spots (→ See `references/troubleshooting/GUIDE.md`)
...
**When external:** optimize that component instead — query tuning, caching, connection pools, circuit breakers (→ See `references/database/GUIDE.md`, [Caching Patterns](references/caching.md)).
...
2. **Write an atomic benchmark** — isolate one function per benchmark to avoid result contamination (→ See `references/benchmark/GUIDE.md`)
...
| Lock contention | mutex/block profile hot | → See `references/concurrency/GUIDE.md` |
| Slow queries | DB time dominates traces | → See `references/database/GUIDE.md` |
...
Automate benchmark comparison in CI to catch regressions before they reach production. → See `references/benchmark/GUIDE.md` for `benchdiff` and `cob` setup.

## Cross-References

- → See `references/benchmark/GUIDE.md` for benchmarking methodology, `benchstat`, and `b.Loop()` (Go 1.24+)
- → See `references/troubleshooting/GUIDE.md` for pprof workflow, escape analysis diagnostics, and performance debugging
- → See `references/data-structures/GUIDE.md` for slice/map preallocation and `strings.Builder`
- → See `references/concurrency/GUIDE.md` for worker pools, `sync.Pool` API, goroutine lifecycle, and lock contention
- → See `references/safety/GUIDE.md` for defer in loops, slice backing array aliasing
- → See `references/database/GUIDE.md` for connection pool tuning and batch processing
- → See `references/observability/GUIDE.md` for continuous profiling in production
