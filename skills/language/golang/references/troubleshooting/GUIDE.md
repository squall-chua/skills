...
1. **Trace callers** — who calls this function and with what values? Call sites can be found with code search tools. → See `references/gopls/GUIDE.md` to resolve the actual symbol through interfaces and embedding — it finds indirect call sites and skips unrelated same-named identifiers that plain grep would respectively miss or falsely match.
...
- → See `references/performance/GUIDE.md` for optimization patterns after identifying bottlenecks
- → See `references/observability/GUIDE.md` for metrics, alerting, and Grafana dashboards for Go runtime monitoring
- → See `samber/cc-skills@promql-cli` skill for querying Prometheus metrics during production incident investigation
- → See `references/concurrency/GUIDE.md`, `references/safety/GUIDE.md`, `references/error-handling/GUIDE.md`s
