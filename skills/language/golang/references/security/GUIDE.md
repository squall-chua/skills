...
Security-relevant linters: `bodyclose`, `sqlclosecheck`, `nilerr`, `errcheck`, `govet`, `staticcheck`. See the `references/lint/GUIDE.md` for configuration and usage.
...
To check the known CVEs of a specific module or version without scanning the whole tree (e.g. when vetting a dependency on pkg.go.dev), → See `references/pkg-go-dev/GUIDE.md`.
...
See `references/database/GUIDE.md`, `references/safety/GUIDE.md`, `references/observability/GUIDE.md`, `references/continuous-integration/GUIDE.md`s.
