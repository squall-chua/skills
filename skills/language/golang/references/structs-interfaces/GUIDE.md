...
> **Community default.** A company skill that explicitly supersedes `references/structs-interfaces/GUIDE.md` takes precedence.
...
→ See `references/naming/GUIDE.md` for interface naming conventions (method + "-er" suffix, canonical names)
...
- → See `references/naming/GUIDE.md` for interface naming conventions (Reader, Closer, Stringer)
- → See `references/design-patterns/GUIDE.md` for functional options, constructors, and builder patterns
- → See `references/dependency-injection/GUIDE.md` for DI patterns using interfaces
- → See `references/code-style/GUIDE.md` for value vs pointer function parameters (distinct from receivers)
- → See `references/gopls/GUIDE.md` for safe rename and the `implementInterface` code action — renaming a method or receiver that participates in interface satisfaction updates every call site and refuses a rename that would silently break the interface, which grep/sed cannot detect
