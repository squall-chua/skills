...
→ See `references/design-patterns/GUIDE.md` for constructor and dependency-injection patterns this move builds on, and [catalog.md](catalog.md) in this skill for the Sprout/Wrap mechanics that typically pair with a freshly introduced seam.
...
- → See `references/testing/GUIDE.md` for general test-writing craft, race-detector mechanics, and `testing/synctest` — this file assumes them as a baseline and only covers when a refactor's safety net needs to reach for them.
- → See `references/benchmark/GUIDE.md` for interpreting a `benchstat` delta and the full profiling methodology.
