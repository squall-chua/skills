...
> **Community default.** A company skill that explicitly supersedes `references/design-patterns/GUIDE.md` takes precedence.

# Go Design Patterns & Idioms

Idiomatic Go patterns for production-ready code. For error handling details see the `references/error-handling/GUIDE.md`; for context propagation see `references/context/GUIDE.md`; for struct/interface design see `references/structs-interfaces/GUIDE.md`.
...
12. **Use `strings.Builder`** for concatenation in loops → see `references/code-style/GUIDE.md`
...
17. **Use `crypto/rand`** for keys/tokens — `math/rand` is predictable → see `references/security/GUIDE.md`
...
→ See `references/structs-interfaces/GUIDE.md` for the `var _ Interface = (*Type)(nil)` pattern.

## Error Flow Patterns

Error cases MUST be handled first with early return — keep the happy path at minimal indentation. → See `references/code-style/GUIDE.md` for the full pattern and examples.
...
Retry logic MUST check `ctx.Err()` between attempts and use exponential/linear backoff via `select` on `ctx.Done()`. Long loops MUST check `ctx.Err()` periodically. → See `references/context/GUIDE.md`.

## Database Patterns

→ See `references/database/GUIDE.md` for sqlx/pgx, transactions, nullable columns, connection pools, repository interfaces, testing.
...
- **Respect 12-factor app** principles — → see `references/project-layout/GUIDE.md`
...
- → See `references/data-structures/GUIDE.md` for data structure selection, internals, and container/ packages
- → See `references/error-handling/GUIDE.md` for error wrapping, sentinel errors, and the single handling rule
- → See `references/structs-interfaces/GUIDE.md` for interface design and composition
- → See `references/concurrency/GUIDE.md` for goroutine lifecycle and graceful shutdown
- → See `references/context/GUIDE.md` for timeout and cancellation patterns
- → See `references/project-layout/GUIDE.md` for architecture and directory structure
- → See `references/refactoring/GUIDE.md` for safely staging a migration toward one of these patterns (options struct, DI, consumer-side interfaces) across an existing codebase
