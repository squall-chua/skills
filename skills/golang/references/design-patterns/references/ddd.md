...
For large systems, each context can be its own Go module in a workspace (`go.work`). See the `references/project-layout/GUIDE.md` for workspace setup.

## Wiring

Wire dependencies in `cmd/server/main.go` using manual constructor injection. → See `references/dependency-injection/GUIDE.md` for DI library alternatives.
