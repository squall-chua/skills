> **Community default.** A company skill that explicitly supersedes `references/context/GUIDE.md` takes precedence.
...
- → See the `references/concurrency/GUIDE.md` for goroutine cancellation patterns using context
- → See the `references/database/GUIDE.md` for context-aware database operations (QueryContext, ExecContext)
- → See the `references/observability/GUIDE.md` for trace context propagation with OpenTelemetry
- → See the `references/design-patterns/GUIDE.md` for timeout and resilience patterns

## Enforce with Linters

Many context pitfalls are caught automatically by linters: `govet`, `staticcheck`. → See the `references/lint/GUIDE.md` for configuration and usage.
