...
Built-in and standard library data structures: internals, correct usage, and selection guidance. For safety pitfalls (nil maps, append aliasing, defensive copies) see `references/safety/GUIDE.md`. For channels and sync primitives see `references/concurrency/GUIDE.md`. For string/byte/rune choice see `references/design-patterns/GUIDE.md`.
...
A slice is a 3-word header: pointer, length, capacity. Multiple slices can share a backing array (→ see `references/safety/GUIDE.md` for aliasing traps and the header diagram).
...
Key functions: `Sort`/`SortFunc`, `BinarySearch`, `Contains`, `Compact`, `Grow`. For `Clone`, `Equal`, `DeleteFunc` → see `references/safety/GUIDE.md`.
...
For `Clone`, `Equal`, sorted iteration → see `references/safety/GUIDE.md`.
...
When using third-party libraries, refer to their official documentation and code examples for current API signatures. For Go package docs, symbols, versions, importers, and known vulnerabilities, → See `references/pkg-go-dev/GUIDE.md` (`godig`) — prefer it over Context7 for Go package facts. To navigate this library's usage in your own code (definitions, call sites, diagnostics), → See `references/gopls/GUIDE.md` (`gopls`). Context7 remains a fallback for docs not indexed on pkg.go.dev.

## Cross-References

- → See `references/performance/GUIDE.md` for struct field alignment, memory layout optimization, and cache locality
- → See `references/safety/GUIDE.md` for nil map/slice pitfalls, append aliasing, defensive copying, `slices.Clone`/`Equal`
- → See `references/concurrency/GUIDE.md` for channels, `sync.Map`, `sync.Pool`, and all sync primitives
- → See `references/design-patterns/GUIDE.md` for `string` vs `[]byte` vs `[]rune`, iterators, streaming
- → See `references/structs-interfaces/GUIDE.md` for struct composition, embedding, and generics vs `any`
- → See `references/code-style/GUIDE.md` for slice/map initialization style
