...
> **Community default.** A company skill that explicitly supersedes `references/testing/GUIDE.md` takes precedence.
...
→ See `references/benchmark/GUIDE.md` for advanced benchmarking: `b.Loop()` (Go 1.24+), `benchstat`, profiling from benchmarks, and CI regression detection.
...
Many test best practices are enforced automatically by linters: `thelper`, `paralleltest`, `testifylint`. See the `references/lint/GUIDE.md` for configuration and usage.

## Cross-References

- -> See `references/stretchr-testify/GUIDE.md` for detailed testify API (assert, require, mock, suite)
- -> See `references/database/GUIDE.md` (testing.md) for database integration test patterns
- -> See `references/concurrency/GUIDE.md` for goroutine leak detection with goleak
- -> See `references/continuous-integration/GUIDE.md` for CI test configuration and GitHub Actions workflows
- -> See `references/lint/GUIDE.md` for testifylint and paralleltest configuration
