...
> **Community default.** A company skill that explicitly supersedes `references/observability/GUIDE.md` takes precedence.
...
See `references/error-handling/GUIDE.md` for the single handling rule. See `references/troubleshooting/GUIDE.md` for using observability signals to diagnose production issues. See `references/security/GUIDE.md` for protecting pprof endpoints and avoiding PII in logs. See `references/context/GUIDE.md` for propagating trace context across service boundaries. See `samber/cc-skills@promql-cli` skill for querying and exploring PromQL expressions against Prometheus from the CLI.
