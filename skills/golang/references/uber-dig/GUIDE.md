...
**Choose fx** for long-running services (HTTP servers, workers, daemons) — lifecycle and signal handling are non-negotiable there. See `references/uber-fx/GUIDE.md`.
...
dig has **no built-in lifecycle**. If you need OnStart/OnStop hooks, signal handling, and graceful shutdown, use fx — see `references/uber-fx/GUIDE.md`.
...
- → See `references/uber-fx/GUIDE.md` for application lifecycle, modules, and signal-aware Run() built on top of dig
