...
→ See `references/design-patterns/GUIDE.md` for detailed architecture guides with file trees and code examples.

## Dependency Injection: Ask Next

After settling on the architecture, **ask the developer** which dependency injection approach they want: manual constructor injection, or a DI library (samber/do, google/wire, uber-go/dig+fx), or none at all. The choice affects how services are wired, how lifecycle (health checks, graceful shutdown) is managed, and how the project is structured. See the `references/dependency-injection/GUIDE.md` for a full comparison and decision table.
...
Packages MUST be lowercase, singular, and match their directory name. → See `references/naming/GUIDE.md` for complete package naming conventions and examples.
...
- **.golangci.yml** — linter config. See the `references/lint/GUIDE.md` for the recommended configuration
...
- [ ] **Ask the developer** their preferred DI approach — see `references/dependency-injection/GUIDE.md`
...
→ See `references/cli/GUIDE.md` for CLI tool structure and Cobra/Viper patterns. → See `references/dependency-injection/GUIDE.md` for DI approach comparison and wiring. → See `references/lint/GUIDE.md` for golangci-lint configuration. → See `references/continuous-integration/GUIDE.md` for CI/CD pipeline setup. → See `references/design-patterns/GUIDE.md` for architectural patterns. → See `references/refactoring/GUIDE.md` for safely moving or splitting existing code into the layout above via type-alias gradual code repair and staged PRs, without a big-bang break.
