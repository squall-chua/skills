...
→ See `references/benchmark/GUIDE.md` (investigation-session.md) for the full runtime metrics table, investigation session setup (scrape interval tuning, env-var toggling), and cost warnings for profiling tools.
...
→ See `references/observability/GUIDE.md` for recommended community Grafana dashboards that visualize Go runtime metrics out of the box.
