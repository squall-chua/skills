...
**Choose raw dig** when you need wiring without a framework: CLI tools, libraries that expose a container to callers, test harnesses, or embedding DI into an existing app that manages its own lifecycle. See `references/uber-dig/GUIDE.md`.
...
- → See `references/uber-dig/GUIDE.md` for the underlying container, `dig.In`/`dig.Out`, and DI without lifecycle
- → See `references/dependency-injection/GUIDE.md` for DI concepts and library comparison
- → See `references/samber-do/GUIDE.md` for a generics-based alternative without reflection
- → See `references/google-wire/GUIDE.md` for compile-time DI (no runtime container)
- → See `references/structs-interfaces/GUIDE.md` for interface design patterns
- → See `references/context/GUIDE.md` for context propagation in OnStart/OnStop hooks
- → See `references/testing/GUIDE.md` for general testing patterns
