...
Maps MUST NOT be accessed concurrently — → see `references/concurrency/GUIDE.md` for sync primitives.
...
For integer overflow as a security vulnerability, see the `references/security/GUIDE.md` section.
...
→ See `references/concurrency/GUIDE.md` for goroutine lifecycle and leak prevention.
...
→ See `references/design-patterns/GUIDE.md` for why init() should be avoided in favor of explicit constructors.

## Enforce with Linters

Many safety pitfalls are caught automatically by linters: `errcheck`, `forcetypeassert`, `nilerr`, `govet`, `staticcheck`. See the `references/lint/GUIDE.md` for configuration and usage.
...
- → See `references/concurrency/GUIDE.md` for concurrent access patterns and sync primitives
- → See `references/data-structures/GUIDE.md` for slice/map internals, capacity growth, and container/ packages
- → See `references/error-handling/GUIDE.md` for nil error interface trap
- → See `references/security/GUIDE.md` for security-relevant safety issues (memory safety, integer overflow)
- → See `references/troubleshooting/GUIDE.md` for debugging panics and race conditions
...
| Multiple `init()` with ordering assumptions | `init()` execution order across files is unspecified. → See `references/design-patterns/GUIDE.md` — use explicit constructors |
